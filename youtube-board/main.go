...
func page(fname string) *template.Template {
	tmps, _ := template.ParseFiles("etmplate/"+fname+".html", "templates/head.html", "templates/foot.html")
	return tmps
}
...
func index(w http.ResponseWriter, rq *http.Request) {
	...
	var pl []my.Post
	db.Where("group_id > 0").Order("created_at desx").Limit(10).Find(&pl)
	var gl []my.Group
...
func post(w http.ResponseWriter, rq *http.Request) {
	...
	db.Table("comments").Select(
		"comments.*, users.id, users.name",
	).Joins(
...
func home(w http.ResponseWriter, rq *http.Request) {
	...
	if rq.Method == "POST" {
		...
		case "post":
			...
			if strings.HasSuffix(ad, "https://youtu.be/") {
